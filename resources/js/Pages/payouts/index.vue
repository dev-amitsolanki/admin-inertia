<script setup>
    ...
    import DeletePayoutForm from './Partials/DeletePayoutForm.vue'
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-indigo-600 leading-tight mb-5">Payout List
                <!-- <PrimaryButton >Save</PrimaryButton> -->
                <Link class="float-right px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-800 " :href="route('payouts.create')" >Add Payout</Link>
            </h2>
                    ...
                    <table class="table border w-full text-sm">
                        <thead class="bg-indigo-100">
                            <tr class="text-left">
                                ...
                                <th class="border p-3">Commision Percentage</th>
                        ...
                        <tbody>
                            <tr v-for="payout in payouts.data" :key="payout.id">
                                <td class="border p-3">{{ payout.id }}</td>
                                <td class="border p-3">{{ payout.user.email }}</td>
                                <td class="border p-3">{{ payout.commission_percentage }}</td>
                                <td class="border p-3 text-center">{{ payout.paid ? 'Paid': 'Pending' }}</td>
                                <td class="border p-3 text-center">{{ payout.total }}</td>
                                <td class="border p-3 text-center">
                                    <a class="flex items-center justify-center text-green-600" target="_blank" :href="payout.stripe_link">
                                ...
                                <td class="border p-3 flex items-center justify-center">                                    
                                    <PrimaryButton class="bg-transparent hover:bg-transparent focus:bg-transparent">
                                        <Link :href="route('payouts.edit',payout.id)">
                                    ...
                                    <DeletePayoutForm :payout="payout"></DeletePayoutForm>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                    <Pagination class="mt-6" :links="payouts.links" />
                </div>
            </layout>
        </template>
    </AuthenticatedLayout>
